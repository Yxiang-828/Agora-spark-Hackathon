...
import {startCodespaceWsBridge} from './ws_bridge';
...
//
// Native port of the plugin's CodespacePanel — same code, de-jailed from the RHS into a
// full-width routed surface (/:team/codespace). The plugin id is fixed (no `manifest` module
// natively), and the plugin's WS relay events are bridged onto the app WebSocketClient.

const PLUGIN_ID = 'com.aegis.agora';
...
const CodespaceApp = () => {
    ...
    // Route the plugin's WS relay events (cs_doc_update/awareness/presence/term_busy/activity/comments)
    // onto the open docs/buses. Idempotent — safe to call on every mount.
    useEffect(() => {
        startCodespaceWsBridge();
    }, []);
